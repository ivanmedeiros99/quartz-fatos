#!/usr/bin/env python3
"""
QUARTZ FIA — Relatórios via GitHub Pages

Consulta a CVM, gera HTML responsivo e publica no GitHub Pages.
Acesse pelo celular: https://SEU_USUARIO.github.io/quartz-fatos/
...
Uso:
  ...
  python quartz_servidor.py --local            # modo local (sem push)

Dependências:
  pip install requests pdfplumber
  Git instalado e configurado (git-scm.com)
"""

import sys, io, re, copy, socket, subprocess, argparse, webbrowser
...
PASTA_SAIDA = Path(__file__).parent / "docs"  # GitHub Pages serve a pasta /docs
...
# ---------------------------------------------------------------------------
# Deploy: Git push para GitHub Pages
# ---------------------------------------------------------------------------
def _git_run(*args) -> bool:
    """Executa um comando git e retorna True se deu certo."""
    try:
        ...
        print("  Git não encontrado. Instale em: https://git-scm.com/downloads")
        return False


def publicar_github():
    """Faz commit e push do HTML para o GitHub."""
    print("\nPublicando no GitHub Pages...")

    _git_run("add", "docs/index.html")
    data_hora = datetime.now().strftime("%d/%m/%Y %H:%M")
    _git_run("commit", "-m", f"Atualização {data_hora}")

    if _git_run("push", "origin", "main"):
        # Descobrir URL do GitHub Pages
        try:
            ...
            # Converter https://github.com/user/repo.git → https://user.github.io/repo/
        ...
        print("  Push realizado! Verifique a URL do GitHub Pages nas Settings do repositório.")
    else:
        print("  Falha no push. Verifique se o repositório remoto está configurado.")

    return None


# ---------------------------------------------------------------------------
# Modo local (servidor HTTP simples)
# ---------------------------------------------------------------------------
def iniciar_servidor_local(porta: int):
    from http.server import HTTPServer, SimpleHTTPRequestHandler

    class Handler(SimpleHTTPRequestHandler):
        def __init__(self, *a, **kw):
            super().__init__(*a, directory=str(PASTA_SAIDA), **kw)
    ...
    print(f"  Computador:  http://localhost:{porta}/index.html")
    print(f"  Celular:     http://{ip}:{porta}/index.html")
...
def main():
    ...
    parser.add_argument("--local", action="store_true", help="Modo local (servidor HTTP, sem GitHub)")
    ...
    # 3. Publicar ou servir localmente
    if args.local:
        webbrowser.open(f"http://localhost:{args.porta}/index.html")
        iniciar_servidor_local(args.porta)
    else:
        url = publicar_github()
        if url:
            webbrowser.open(url)
