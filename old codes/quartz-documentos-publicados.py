...
TIPOS_DOCUMENTO = {
    ...
    "resultados": {
        "codigo": "IPE_6_-1_-1",       # Dados Econômico-Financeiros
        "label": "Release de Resultados",
        "label_curto": "RR",
        "sempre_gerar": False,          # só gera se tiver registros
    },
}
...
# ---------------------------------------------------------------------------
def consultar_rad(data_alvo: date, codigo_categoria: str) -> tuple[list[dict], list[dict]]:
...
def main():
    ...
    for chave, tipo in TIPOS_DOCUMENTO.items():
        print(f"\n[{tipo['label_curto']}] Consultando: {tipo['label']}...")

        try:
            raw_portfolio, raw_outros = consultar_rad(data_alvo, tipo["codigo"])
