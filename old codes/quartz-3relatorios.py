...
# Após testes, o código para buscar TODOS os documentos eventuais (IPE)
# é "IPE_-1_-1_-1". Fazemos uma única consulta ampla e filtramos no código
# pelo texto da coluna "Categoria" retornada pela CVM.
#
# Isso resolve o problema de códigos numéricos desconhecidos e garante
# que nenhum documento seja perdido.
# ---------------------------------------------------------------------------
TIPOS_DOCUMENTO = {
    "fatos_relevantes": {
    ...
    "comunicados": {
        ...
        # Excluir apresentações a analistas (vão para o relatório de resultados)
    ...
    "resultados": {
        ...
        # Manter apenas press-releases e apresentações
...
def consultar_tipo(data_alvo: date, tipo_config: dict) -> tuple[list[dict], list[dict]]:
    """
    Faz uma consulta ampla (todos os docs eventuais) e filtra pelo texto
    da coluna Categoria + palavras-chave definidas em tipo_config.
    Retorna (fatos_portfolio, fatos_outros).
    """
    print(f"    Consultando RAD/CVM...")
    todos_raw = _consulta_unica(data_alvo, "IPE_-1_-1_-1")

    # Filtro 1: categoria
    cats = tipo_config.get("filtro_categoria", [])
    if cats:
        todos_raw = [f for f in todos_raw if any(c in f.get("categoria", "") for c in cats)]

    # Filtro 2: manter apenas docs com certas palavras
    manter = tipo_config.get("manter_palavras")
    if manter:
        todos_raw = [f for f in todos_raw if any(p in _texto_buscavel(f) for p in manter)]

    # Filtro 3: excluir docs com certas palavras
    excluir = tipo_config.get("excluir_palavras")
    if excluir:
        todos_raw = [f for f in todos_raw if not any(p in _texto_buscavel(f) for p in excluir)]

    # Separar portfolio vs outros
    portfolio_fatos, outros_fatos = [], []
    for fato in todos_raw:
