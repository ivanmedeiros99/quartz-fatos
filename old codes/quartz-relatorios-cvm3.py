...
# Matching feito por NOME (razão social parcial) — mais confiável que código CVM
# ---------------------------------------------------------------------------
PORTFOLIO = {
    "ASAI3":  {"nomes": ["SENDAS DISTRIBUIDORA", "ASSAI", "ASSAÍ"],                     "cnpj": "03.560.312/0001-24"},
    "AXIA6":  {"nomes": ["CENTRAIS ELET", "ELETROBRAS", "ELETROBRÁS", "AXIA ENERGIA"],  "cnpj": "00.001.180/0001-26"},
    "CEAB3":  {"nomes": ["C&A MODAS", "CEA MODAS", "C&amp;A MODAS"],                    "cnpj": "45.242.914/0001-05"},
    "CSAN3":  {"nomes": ["COSAN"],                                                       "cnpj": "50.746.577/0001-15"},
    "EQTL3":  {"nomes": ["EQUATORIAL S", "EQUATORIAL ENERGIA"],                          "cnpj": "03.220.438/0001-73"},
    "GGPS3":  {"nomes": ["GPS PARTICIPAC"],                                               "cnpj": "09.229.201/0001-30"},
    "HAPV3":  {"nomes": ["HAPVIDA"],                                                      "cnpj": "63.554.067/0001-98"},
    "INTB3":  {"nomes": ["INTELBRAS"],                                                    "cnpj": "82.901.000/0001-27"},
    "ITUB4":  {"nomes": ["ITAU UNIBANCO", "ITAÚ UNIBANCO"],                              "cnpj": "60.872.504/0001-23"},
    "MULT3":  {"nomes": ["MULTIPLAN"],                                                    "cnpj": "07.816.890/0001-53"},
    "ORVR3":  {"nomes": ["ORIZON"],                                                       "cnpj": "11.421.994/0001-36"},
    "RADL3":  {"nomes": ["RAIA DROGASIL", "RAIADROGASIL"],                                "cnpj": "61.585.865/0001-51"},
    "RENT3":  {"nomes": ["LOCALIZA"],                                                     "cnpj": "16.670.085/0001-55"},
    "SMFT3":  {"nomes": ["SMARTFIT", "SMART FIT"],                                        "cnpj": "07.594.978/0001-78"},
}


def _identificar_ticker(nome_empresa: str) -> str | None:
    """Identifica o ticker pelo nome da empresa (case-insensitive, match parcial)."""
...
def consultar_tipo(data_alvo: date, tipo_config: dict) -> tuple[list[dict], list[dict]]:
    ...
    # Separar portfolio vs outros — por nome da empresa
    portfolio_fatos, outros_fatos = [], []
    for fato in todos:
        ticker = _identificar_ticker(fato["nome"])
        if ticker:
            fato["ticker"] = ticker
...
def _bloco_portfolio(pdf, fato):
    ...
    if fato.get("tipo_doc"):
        meta += f"  |  Tipo: {fato['tipo_doc']}"
...
def _bloco_outros(pdf, fato):
    ...
    meta = f"Assunto: {fato.get('assunto', 'N/D')}  |  Entrega: {fato.get('data_entrega', '')}  |  CVM: {fato.get('cod_cvm_raw', '')}"
    if fato.get("tipo_doc"):
        meta += f"  |  Tipo: {fato['tipo_doc']}"
    pdf.cell(0, 4, meta, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    if fato.get("link"):
