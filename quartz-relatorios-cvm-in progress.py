...
def _carregar_tickers_b3() -> None:
    """Baixa a lista de empresas listadas na B3 e popula _TICKERS_B3."""
    global _TICKERS_B3
    if _TICKERS_B3:
        return  # já carregado
    try:
        ...
        empresas = resp.json()
        # codeCVM é string sem zeros à esquerda; issuingCompany é o ticker (ex: "HAPV3")
        _TICKERS_B3 = {
            str(e["codeCVM"]): e["issuingCompany"].strip()
            for e in empresas
            if e.get("codeCVM") and e.get("issuingCompany")
        }
...
def _ticker_b3(cod_raw: str) -> str:
    """Converte '02537-2' → 'ASAI3' usando o mapa B3. Retorna cod_raw se não encontrar."""
    b3_key = cod_raw.replace("-", "").lstrip("0") or "0"
    return _TICKERS_B3.get(b3_key, cod_raw.strip())
...
def _bloco_outros(pdf, fato):
    ...
    pdf.set_font("Arial", "B", 9)
    pdf.set_text_color(26, 54, 93)
    pdf.cell(0, 5, f"[{fato.get('cod_cvm_raw', '')}]  {fato['nome']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
